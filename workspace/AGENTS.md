...
### 觸發條件
當使用者訊息包含以下任一關鍵字時，**啟用簡報製作模式**：
簡報、PPT、PPTX、投影片、slide、presentation、做簡報、製作簡報、
pitch deck、slide deck、做投影片

### 強制執行步驟（不可跳過）

⚠️ **收到簡報需求時，使用 pptx 技能產生 .pptx 檔案。不要用純文字模擬簡報。**
...
**Step 2**: 使用 `exec` 工具執行 Node.js 腳本產生 .pptx
根據 pptx 技能的 pptxgenjs.md 參考，撰寫 Node.js 腳本並執行：
```bash
node -e "
const pptxgen = require('pptxgenjs');
const pres = new pptxgen();
// ... 根據用戶需求生成 slides ...
pres.writeFile({ fileName: '/sandbox/workspace/output/presentation.pptx' });
"
```
...
### PptxGenJS 注意事項
- 顏色代碼**不加 `#` 前綴**（例：`003366` 而非 `#003366`）
- bullet 用 `{ bullet: true }` 而非 Unicode 符號 `•`
- 多行文字用 `{ breakLine: true }`
- **不要重用 option 物件**（PptxGenJS 會 mutate），每次建立新物件
- 輸出到 `/sandbox/workspace/output/`

### 🚫 禁止
- ❌ 不要用純文字列出簡報內容代替實際產生 .pptx
- ❌ 不要用 python-pptx，使用 PptxGenJS（Node.js）
- ❌ 顏色不要加 `#` 前綴
- ❌ 不要用 Unicode bullet 符號
